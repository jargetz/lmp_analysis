...
def main():
    st.set_page_config(
        ...
        page_icon="attached_assets/project2030_favicon.png",
        layout="wide"
    )
    _inject_css()
...
def render_node_map_tab():
    ...
    with left_col:
        ...
        selection_status = (
            st.status(
                f"Loading map and analysis for {selected_name}…",
                expanded=False,
            )
        ...
        if selection_status:
            selection_status.update(
                label=f"Map ready for {selected_name}; loading price analysis…",
                state="running",
            )
    ...
    with right_col:
        ...
        if node_to_analyze is None:
            ...
            if selection_status:
                selection_status.update(
                    label=f"{selected_name} analysis ready",
                    state="complete",
                    expanded=False,
                )
